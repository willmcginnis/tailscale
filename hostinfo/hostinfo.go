...
// SetHostNameFn sets a custom function for querying the hostname.
// The default implementation of os.Hostname is used if not set.
func SetHostNameFn(fn func() (string, error)) {
	hostNameFn.Store(fn)
}

// Hostname returns the hostname of the current host using the function
// set by SetHostNameFn.
